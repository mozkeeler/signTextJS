...
function signText(domWindow, text, caOption) {
  ...
  if (nonRepudiation) {
    ...
    let filteredCerts = [];
    for (let cert of userCerts) {
      let nativeCert = CERT_FindUserCertByUsage(certDB, cert.nickname,
                                                certUsageEmailSigner, true,
                                                null);
  ...
  if (autoSign) {
    certName = {
      error: null,
      nickname: userCerts[0].nickname,
      password: null
    };
  } else {
    certName = selectCert(userCerts, text);
