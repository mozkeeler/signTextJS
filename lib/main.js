...
function addNewSupportsString(text, supportsArray) {
  let supportsString = Cc["@mozilla.org/supports-string;1"]
                         .createInstance(Ci.nsISupportsString);
  supportsString.data = text;
  supportsArray.AppendElement(supportsString);
}
...
function selectCert(userCerts, text) {
  try {
    ...
    let supportsArray = Cc["@mozilla.org/supports-array;1"]
                          .createInstance(Ci.nsISupportsArray);
    addNewSupportsString(domWindow.content.location.hostname, supportsArray);
    addNewSupportsString(text, supportsArray);
    for (let cert of userCerts) {
      addNewSupportsString(cert.nickname, supportsArray);
      let certSubjectName = cert.subjectName;
      if (!certSubjectName) {
        certSubjectName = cert.emailAddress;
      }
      addNewSupportsString(certSubjectName, supportsArray);
      addNewSupportsString(cert.issuerCommonName, supportsArray);
      addNewSupportsString(cert.tokenName, supportsArray);
    }
    let dialog = windowWatcher.openWindow(domWindow,
                                          data.url("certChooser.html"),
                                          "_blank",
...
function signText(text) {
  ...
  try {
    ...
    // XXX TODO: implement picking which certificate, confirmation dialog
    log("using '" + certName);
